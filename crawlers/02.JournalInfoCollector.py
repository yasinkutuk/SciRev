...
part = 50
start = 0
ran1 = len(sitelinks) - (len(sitelinks) % part)
ran2 = (len(sitelinks) % part)

for s in range(start, ran1, part):
    a0 = []
    a1 = []
    a2 = []
    a3 = []
    ...
    for i in range(s, s + part):
        ...
        bar.next()
    bar.finish()
    data = {'Journal link': a0, 'Issues per year': a1, 'Articles published last year': a2,
            'Manuscripts received last year': a3,
            ...
            'Five-year impact factor': a10, 'Duration first review ': a21,
            'Total handling time accepted manuscripts': a22,
            ...
            'Difficulty of reviewer comments': a27,
            'Overall rating manuscript handlingOverall rating manuscript handling': a28}
    data = pd.DataFrame(data)
    data.to_csv(datapath + '02.Reviews' + str(s) + '.csv', sep=';')
    time.sleep(10)
    print ""
    print "10 sn. bekliyor."
    print ""
...
a27 = []
a28 = []
for i in range(ran1, len(sitelinks)):
	br = mec.Browser()
    page = br.open(sitelinks[i])
    tree = html.parse(page)
    get1 = tree.xpath('/html/body/div[2]/div[3]/div[3]/div[1]/dl/dd[1]/text()')
...
data = {'Journal link': a0, 'Issues per year': a1, 'Articles published last year': a2,
        ...
        'Overall rating manuscript handlingOverall rating manuscript handling': a28}
data = pd.DataFrame(data)
data.to_csv(datapath + '02.Reviews' + str(len(sitelinks)) + '.csv', sep=';')
time.sleep(10)
print ""
print "10 sn. bekliyor."
print ""
